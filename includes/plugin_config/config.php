...
/*
    ...
    public static function print_js($version=false) {
        ...
        foreach(self::$js_files as $hash=>$js_file_info){
            if(strlen($js_file_info[2])<3){ // url is set to 'true', use the module/file name combo
	            ?>
                <script type="text/javascript" language="javascript" src="<?php echo _BASE_HREF;?>includes/plugin_<?php echo $js_file_info[0];?>/js/<?php echo $js_file_info[1]; echo ($version && strpos('?',$js_file_info[1])===false) ? '?ver='.$version : ''; ?>"></script> <?php
            }else{ ?>
	...
	public static function js_combine($version, $reinit=false){
		...
		if(is_file($cache_file) && filesize($cache_file) > 10 && module_config::c('js_cached_version',$version) == $version){
			...
			foreach(self::$js_files as $hash=>$js_file_info) {
				if ( strlen( $js_file_info[2] ) < 3 ) {
					if(is_file('includes/plugin_'.basename($js_file_info[0]).'/js/'.basename($js_file_info[1]))){
						$js_content .= "\n";
						$js_content .= " // ".basename($js_file_info[0]).' / '.basename($js_file_info[1])."\n\n";
						$js_content .= file_get_contents('includes/plugin_'.basename($js_file_info[0]).'/js/'.basename($js_file_info[1]));
						$js_content .= "\n";
					}
				}
			}
    ...
    public function get_install_sql(){
        ...
        INSERT INTO `<?php echo _DB_PREFIX;?>config` SET `key` = '_installation_code', `val` = 'free';
