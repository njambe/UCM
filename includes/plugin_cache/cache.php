            ...
            foreach(self::$_db_cache as $cache_group => $data){
                foreach($data as $cache_key => $cache_data){
                    if(is_array($cache_data)){
                        ...
                        $sql = "REPLACE INTO `"._DB_PREFIX."cache_store` SET `cache_group` = '".mysql_real_escape_string($cache_group)."', `cache_key` = '".mysql_real_escape_string($cache_key)."', cache_data = '".mysql_real_escape_string(serialize($cache_data))."', `create_user_id` = ".module_security::get_loggedin_id().", expire_time = ".(int)$cache_data['e'];
                        query($sql);
                    }
                }
            }
    ...
    public static function clear($group,$cache_key=false){
        ...
        if($cache_key){
            ...
            if(self::$_use_memcache){
                ...
                if(self::db_table_exists('cache_store')){
                    $sql = "DELETE FROM `"._DB_PREFIX."cache_store` WHERE `cache_group` = '".mysql_real_escape_string($group)."' AND `cache_key` = '".mysql_real_escape_string($cache_key)."'";
                    query($sql);
                }
                if(isset(self::$_db_cache[$group]) && isset(self::$_db_cache[$group][$cache_key])){
            ...
            if(self::$_use_memcache){
                ...
                $sql = "REPLACE INTO `"._DB_PREFIX."cache` SET `cache_group` = '".mysql_real_escape_string($group)."', `expire_time` = ".time();
                query($sql);
            }else{
                // session mode, remove all cache elements from this item.
                if(self::db_table_exists('cache_store')){
                    $sql = "DELETE FROM `"._DB_PREFIX."cache_store` WHERE `cache_group` = '".mysql_real_escape_string($group)."'";// AND `create_user_id` = ".module_security::get_loggedin_id();
                    query($sql);
                }
