...
define('_CUSTOMER_TYPE_NORMAL',0);
define('_CUSTOMER_TYPE_LEAD',1);
...
','Customer Signup Form',array(
    ...
    public function pre_menu(){
		...
		if($this->can_i('view','Customers')){
			$this->links['customers'] = array(
				...
				"args"=>array('customer_id'=>false,'leads'=>false),
                'icon_name' => 'users',
            ...
            if(in_array('customer',$load_modules)){
                $this->links['customers']['current'] = !self::is_lead();
            }
        }

		if($this->leads_enabled() && $this->can_i('view','Leads')){
			$this->links['leads'] = array(
				"name"=>"Leads",
				"p"=>"customer_admin_list",
				"args"=>array('customer_id'=>false,'leads'=>1),
                'icon_name' => 'users',
			);
            if(in_array('customer',$load_modules)){
                $this->links['leads']['current'] = self::is_lead();
            }
        }

        if(file_exists(dirname(__FILE__).'/pages/customer_signup.php') && module_config::can_i('view','Settings') && $this->can_i('view','Customers')){
    ...
    public static function leads_enabled(){
        return module_config::c('leads_enabled',1);
    }
    public static $checking_leads_customer = false;
    public static function is_lead(){
        if(!self::leads_enabled())return false;
        if(!self::$checking_leads_customer && isset($_REQUEST['customer_id']) && (int)$_REQUEST['customer_id'] > 0){
            self::$checking_leads_customer = true;
            $temp_customer_lead_change = self::get_customer($_REQUEST['customer_id'],false,true);
            self::$checking_leads_customer = false;
            if($temp_customer_lead_change && isset($temp_customer_lead_change['type']) && $temp_customer_lead_change['type'] == _CUSTOMER_TYPE_LEAD){
                return true;
            }else if ($temp_customer_lead_change && isset($temp_customer_lead_change['type']) && $temp_customer_lead_change['type'] == _CUSTOMER_TYPE_NORMAL){
                return false;
            }
        }
        if(isset($_REQUEST['leads']))return true;
        return false;
    }
    ...
    /** static stuff */
     ...
     public static function link_generate($customer_id=false,$options=array(),$link_options=array()){
        ...
        $options['arguments'] = array(
         ...
         if(self::is_lead($customer_id)){
             $options['arguments']['leads'] = 1;
         }
        ...
        if(!self::can_i('view','Customers') && (!isset($options['skip_permissions']) || !$options['skip_permissions'])){
            if(!isset($options['full']) || !$options['full']){
                $link = '#';
            }else{
...
//                }
	...
	public static function get_customer($customer_id,$skip_permissions=false,$basic_for_link=false){
        ...
        if(!$customer){
            $customer = array(
                ...
                'staff_ids' => array(),
                'type' => self::is_lead() ? _CUSTOMER_TYPE_LEAD : _CUSTOMER_TYPE_NORMAL,
            );
        }
    ...
    public static function handle_import_leads($data,$add_to_group){
        foreach($data as $rowid => $row){
            $data[$rowid]['type'] = _CUSTOMER_TYPE_LEAD;
        }
        self::handle_import($data,$add_to_group);
    }
    ...
    public function get_upgrade_sql(){
        ...
        if(!isset($fields['type'])){
            $sql .= 'ALTER TABLE  `'._DB_PREFIX.'customer` ADD  `type` tinyint(2) NOT NULL DEFAULT \'0\' AFTER `default_invoice_prefix`;';
        }
...
INSERT INTO `<?php echo _DB_PREFIX; ?>customer` VALUES (1, 3, 0, 'Bobs Printing Service', 0, -1, '', '', 0, NOW(), NOW());
INSERT INTO `<?php echo _DB_PREFIX; ?>customer` VALUES (2, 4, 0, 'Richards Roof Repairs', 0, -1, '', '', 0, NOW(), NOW());
    ...
    public function external_hook($hook){
        ...
        switch($hook){
            ...
            case 'public_signup':
				...
				$contact_fields = array('name','last_name','email','phone','password');
