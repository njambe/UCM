...
if(module_customer::is_lead()){
    ...
    $search['type'] = _CUSTOMER_TYPE_LEAD;
}else{
    $search['type'] = _CUSTOMER_TYPE_NORMAL;
}
...
if(module_customer::can_i('create',$page_type)){
    $header_buttons[] = array(
        'url' => module_customer::link_open('new',false,array('leads'=>module_customer::is_lead())),
...
if(class_exists('module_import_export',false) && module_customer::can_i('view','Import '.$page_type)){
    $header_buttons[] = array(
        'url' => module_import_export::import_link(
                array(
                    'callback'=>'module_customer::handle_import'.(module_customer::is_lead() ? '_leads' : ''),
                    'name'=>$page_type,
