...
<form action="" method="post" id="invoice_form">
    ...
    $fieldset_data = array(
        ...
        'extra_settings' => array(
            ...
            'layout' => 'table_row',
             'allow_new' => module_invoice::can_i('create','Invoices'),
             'allow_edit' => module_invoice::can_i('edit','Invoices'),
        ),
    );
    $incrementing = false;
            ...
            <tr>
                ...
                <td>
                    ...
                    foreach($payment_methods as $payment_method){
                        if($payment_method->is_enabled() && $payment_method->is_allowed_for_invoice($invoice_id) && $payment_method->is_method('online')){ ?>
                    ...
                    foreach($payment_methods as $payment_method){
                        if($payment_method->is_enabled() && $payment_method->is_allowed_for_invoice($invoice_id) && $payment_method->is_method('offline')){ ?>
         ...
         ?>
            <div class="tableclass_form content">
                ...
                <?php }else{ ?>
                    ...
                    <p align="center">
                    <input type="submit" name="butt_email" id="butt_email_overdue" value="<?php echo _l('Email Invoice'); ?>" class="submit_button" />
                    </p>
                <?php } ?>
            </div>
        <?php
                        ...
                        <tr>
                            ...
                            <th width="10%">
                                <?php if($invoice['default_task_type']==_TASK_TYPE_AMOUNT_ONLY){
                                    ...
                                    _e(module_config::c('task_qty_name','Qty'));
                                }else if($invoice['default_task_type']==_TASK_TYPE_HOURS_AMOUNT){
                                    _e(module_config::c('task_hours_name','Hours'));
                                } ?>
                            </th>
                            <th width="10%">
                                <?php if($invoice['default_task_type']==_TASK_TYPE_AMOUNT_ONLY){
                                    _e(module_config::c('invoice_amount_name','Amount'));
                                }else if($invoice['default_task_type']==_TASK_TYPE_QTY_AMOUNT){
                                    _e(module_config::c('invoice_amount_name','Amount'));
                                }else if($invoice['default_task_type']==_TASK_TYPE_HOURS_AMOUNT){
                                    _e(module_config::c('invoice_rate_name','Rate'));
                                } ?>
